...
from acesfldefense.defense import (
    screen_updates,
    update_trust,
)

from acesfldefense.compression import (
...
class ResourceAwareFedAvg(FedAvg):
    ...
    def aggregate_train(
        ...
        # =================================================
        # ACES-FL TRUST / ANOMALY DEFENSE
        #
        # Phase 1:
        #   Decompress and collect ALL client updates.
        #
        # Phase 2:
        #   Build robust round consensus and score each
        #   update using norm, cosine direction and trust.
        #
        # Phase 3:
        #   Aggregate ACCEPTED updates only.
        #
        # The experimental malicious label is NEVER used
        # to make the defense decision. It is used only
        # afterwards to calculate detection performance.
        # =================================================
        ...
        # ---------------------------------------------
        # Phase 1: decompress and collect
        # ---------------------------------------------
        ...
        if not hasattr(
            ...
            self._aces_trust_scores = {}

        # ---------------------------------------------
        # Phase 2: anomaly screening
        # ---------------------------------------------

        screening = screen_updates(
            updates=[
                item["delta"]
        ...
        write_defense_header = (
            not defense_path.exists()
        )
        ...
        # ---------------------------------------------
        # Phase 3: accept/reject and aggregate
        # ---------------------------------------------


        with defense_path.open(
            "a",
            newline="",
            encoding="utf-8",
        ) as defense_file:

            defense_writer = csv.writer(
                defense_file
            )

            if write_defense_header:
                defense_writer.writerow(
                    [
                        "round",
                        "resource_node_label",
                        "node_id",
            ...
            for item in client_updates:
                ...
                trust_after = update_trust(
                    old_trust=trust_before,
                    accepted=accepted,
                    alpha=0.80,
                )
                ...
                # Malicious truth is NOT used above.
                # -------------------------------------
        ...
        print(
            f"DEFENSE CONSENSUS ROUND "
            f"{server_round}: "
            f"median_norm="
            f"{screening['median_norm']:.6f} | "

            f"MAD="
            f"{screening['mad_norm']:.6f}"
        )
        ...
        # ---------------------------------------------
        # ACES defense effectiveness metrics
        # ---------------------------------------------

        aggregated_metrics[
            "defense_accepted_clients"
        ] = int(accepted_count)

        aggregated_metrics[
            "defense_rejected_clients"
        ] = int(rejected_count)

        aggregated_metrics[
            "defense_true_positive"
        ] = int(true_positive)
        ...
        aggregated_metrics[
            "defense_false_negative"
        ] = int(false_negative)

        aggregated_metrics[
            "defense_true_negative"
        ] = int(true_negative)
